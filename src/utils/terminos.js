// src/data/terminosCondiciones.js

const terminosCondiciones = {
  titulo: "📜 Términos y Condiciones de Uso – TeLoRecargo.com®",
  subtitulo: "Un producto de Tecnologías Administrativas ELAD®",
  secciones: [
    {
      titulo: "1️⃣ Objeto del servicio",
      contenido:
        "TeLoRecargo.com® es una plataforma digital desarrollada por Tecnologías Administrativas ELAD® que permite a usuarios registrados vender recargas de tiempo aire electrónico de forma rápida, sencilla y desde cualquier dispositivo con acceso a internet.\n📌 Este sistema está enfocado exclusivamente en la venta de recargas de tiempo aire. No incluye la venta de productos o servicios como pagos de CFE, Telmex, etc.",
    },
    {
      titulo: "✅ 2️⃣ Aceptación",
      contenido:
        "El uso del sistema implica la aceptación plena y sin reservas de estos Términos y Condiciones. Al registrarse, el usuario reconoce haberlos leído y comprendido.",
    },
    {
      titulo: "🧑‍💻 3️⃣ Registro y uso de la cuenta",
      contenido:
        "Para operar en TeLoRecargo.com®, el Usuario debe:\n• Registrarse con datos reales y verificables.\n• Crear usuario y contraseña bajo su responsabilidad.\n• Tener acceso a internet en su dispositivo.",
    },
    {
      titulo: "💳 4️⃣ Proceso de compra y uso de saldo",
      contenido:
        "• Las compras se realizan mediante depósito o transferencia.\n• Se debe subir el comprobante (captura) a la plataforma.\n• Una vez validado, el saldo se abona en menos de 30 minutos (en horario laboral).\n⚠️ El saldo es prepagado, no reembolsable y se usa solo para recargas.",
    },
    {
      titulo: "🎁 5️⃣ Bonificación por compra",
      contenido:
        "• TLR otorga un 5% en cada compra.\n• Este bono puede cambiar o suspenderse sin previo aviso.\n💡 Puede llegar a 10% segun se cumpla con los requisitos.",
    },
    {
      titulo: "🧾 6️⃣ Autofacturación",
      contenido:
        "• El usuario puede generar su factura desde el panel.\n• Las facturas se emiten bajo el RFC registrado.\n• Deben generarse dentro del mes fiscal correspondiente.",
    },
    {
      titulo: "🔒 7️⃣ Responsabilidad del usuario",
      contenido:
        "El usuario debe:\n• Verificar el número y monto antes de recargar.\n• Subir comprobantes válidos.\n• No duplicar ni manipular pagos.\n• Mantener confidencial su acceso.\n🚫 El mal uso implica bloqueo o cancelación sin reembolso.",
    },
    {
      titulo: "⚠️ 8️⃣ Límites de responsabilidad",
      contenido:
        "TLR no se responsabiliza por:\n• Problemas bancarios.\n• Fallas de operadores móviles.\n• Errores del Usuario al operar el sistema.",
    },
    {
      titulo: "📞 9️⃣ Soporte y disponibilidad",
      contenido:
        "🕘 Lunes a viernes: 9:00 a.m. – 5:00 p.m.\n🕑 Sábados: 9:00 a.m. – 2:00 p.m.\n📱 Soporte por WhatsApp.\n🕓 El sistema está disponible 24/7, salvo mantenimiento o interrupciones externas.",
    },
    {
      titulo: "🛠️ 🔟 Cambios en los términos",
      contenido:
        "TLR puede modificar estos términos sin previo aviso.\nEl uso continuo de la plataforma implica aceptación automática.",
    },
    {
      titulo: "🔐 1️⃣1️⃣ Protección de datos",
      contenido:
        "Los datos personales se tratan con estricta confidencialidad.\n📜 Solo serán compartidos si lo exige la ley.",
    },
    {
      titulo: "🔚 1️⃣2️⃣ Terminación del servicio",
      contenido:
        "TLR podrá suspender cuentas por:\n• Actividades sospechosas.\n• Incumplimiento de estos términos.\nSin obligación de reembolso.",
    },
    {
      titulo: "⚖️ 1️⃣3️⃣ Jurisdicción",
      contenido:
        "Cualquier controversia será resuelta conforme a las leyes mexicanas, ante tribunales en Acapulco, Guerrero.",
    },
    {
      titulo: "🧠 1️⃣4️⃣ Definiciones clave",
      contenido:
        "• Usuario: Persona registrada en el sistema.\n• Saldo: Crédito prepagado para recargas.\n• TLR: TeLoRecargo.com®\n• Autofactura: CFDI generado por el usuario desde el sistema.",
    },
    {
      titulo: "🛡️ 1️⃣5️⃣ Seguridad de la cuenta",
      contenido:
        "El Usuario es responsable de proteger sus credenciales.\nTLR no asume responsabilidad por accesos no autorizados por negligencia del usuario.",
    },
    {
      titulo: "🚫 1️⃣6️⃣ Fraudes y conductas prohibidas",
      contenido:
        "No está permitido:\n• Usar datos falsos.\n• Subir comprobantes falsos o alterados.\n• Automatizar procesos con bots.\n• Usar el sistema para fines ilícitos.\n⚠️ Las cuentas serán canceladas sin aviso ni reembolso. Posibles acciones legales.",
    },
    {
      titulo: "💼 1️⃣7️⃣ Uso comercial del sistema",
      contenido:
        "Si operas con fines comerciales, deberás declararlo.\nLas cuentas con alto volumen podrían requerir verificación fiscal adicional.",
    },
    {
      titulo: "📜 1️⃣8️⃣ Facturación por terceros",
      contenido:
        "Solo puede facturar el titular del RFC registrado.\nEstá prohibida la facturación cruzada sin autorización.",
    },
    {
      titulo: "🌐 1️⃣9️⃣ Restricciones territoriales",
      contenido:
        "La plataforma está diseñada para uso en territorio mexicano.\nEl acceso desde otros países no está garantizado ni soportado.",
    },
    {
      titulo: "✍️ 2️⃣0️⃣ Aceptación electrónica",
      contenido:
        "Todos los registros digitales y acciones del usuario en la plataforma tienen validez legal equivalente a una firma física bajo la NOM-151 y el Código de Comercio.",
    },
    {
      titulo: "📝 Última actualización: ",
      contenido: `📅 Julio 2025

Al registrarte y utilizar la plataforma TeLoRecargo.com, aceptas los siguientes términos y condiciones relacionados con el esquema de comisiones vigente.

<hr/>

🔢 <strong>1. ESQUEMA DE COMISIONES</strong><br/><br/>
TeLoRecargo.com ofrece un sistema de comisiones por la compra y reventa de tiempo aire para todas las marcas disponibles. Las comisiones se otorgan en base al monto de compra mensual o al plan contratado con MiTiendaEnLineaMX.com.mx.

<h4>📊 1.1 Comisión por Rango de Compra Mensual:</h4>
<table border="1" cellpadding="6" cellspacing="0" style="border-collapse: collapse; width:100%; font-size: 14px;">
  <thead>
    <tr>
      <th>Rango de compra mensual</th>
      <th>Porcentaje de comisión</th>
    </tr>
  </thead>
  <tbody>
    <tr><td>$0 a $4,999 MXN</td><td>5.0%</td></tr>
    <tr><td>$5,000 a $9,999 MXN</td><td>5.5%</td></tr>
    <tr><td>$10,000 a $24,999 MXN</td><td>6.0%</td></tr>
    <tr><td>$25,000 a $49,999 MXN</td><td>6.5%</td></tr>
    <tr><td>$50,000 a $74,999 MXN</td><td>6.7%</td></tr>
    <tr><td>$75,000 a $99,999 MXN</td><td>6.8%</td></tr>
    <tr><td>$100,000 o más</td><td>6.9%</td></tr>
  </tbody>
</table>

<h4>🧾 1.2 Comisión por Plan Contratado (MiTiendaEnLineaMX):</h4>
<table border="1" cellpadding="6" cellspacing="0" style="border-collapse: collapse; width:100%; font-size: 14px;">
  <thead>
    <tr>
      <th>Plan contratado</th>
      <th>Comisión aplicada</th>
      <th>Límite mensual de compra</th>
    </tr>
  </thead>
  <tbody>
    <tr><td>Plan Negocios</td><td>8.0%</td><td>Hasta $5,000 MXN</td></tr>
    <tr><td>Plan Profesional</td><td>10.0%</td><td>Hasta $10,000 MXN</td></tr>
  </tbody>
</table>

📌 Las comisiones por plan contratado no se acumulan con las de compra mensual. Se aplicará la más alta entre ambas, considerando sus respectivos topes.

🎁 <strong>1.3 Comisión de Bienvenida:</strong><br/>
Todo nuevo usuario recibe una comisión del 10% en su primera recarga, sin importar el monto. Esta comisión es única y no acumulable con otros beneficios.

<hr/>

📌 <strong>2. CONDICIONES GENERALES</strong><br/>
• Las comisiones se reflejan al momento de realizar la recarga y pueden consultarse en el historial de transacciones.<br/>
• Para conservar un nivel de comisión mensual, debe mantenerse el volumen de compras correspondiente cada mes.<br/>
• En caso de inactividad mayor a 60 días, la cuenta podrá ser suspendida o reiniciada al nivel mínimo de comisión.

<hr/>

🔄 <strong>3. MODIFICACIONES AL ESQUEMA DE COMISIONES</strong><br/>
TeLoRecargo.com se reserva el derecho de modificar estos términos y el esquema de comisiones sin previo aviso. Cualquier cambio será publicado en la plataforma y notificado por medios oficiales.
`,
},

    {
      titulo: "📬 2️⃣1️⃣ Contacto",
      contenido:
        "📧 contacto@telorecargo.com\n📱 Soporte disponible en WhatsApp desde tu sesión.\nGracias por confiar en una solución 100% mexicana.\nTeLoRecargo.com® – Recarga, factura y gana.",
    },
  ],
};

export default terminosCondiciones;
