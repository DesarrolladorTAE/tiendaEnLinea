...
export function ServiceGeneralStep({ form, setField, errors, imageProps }) {
  return <Stack spacing={3}>
    ...
    <Box>
      ...
      <Grid container spacing={1.5}>{SERVICE_TYPES.map(([value, label, Icon]) => <Grid size={{ xs: 6, sm: 4, md: 3 }} key={value}><Card className="service-type-card" variant="outlined" sx={{ height: "100%", bgcolor: form.service_type === value ? "#fff3d6" : "#fff", color: "#fff", border: "2px solid", borderColor: form.service_type === value ? "#b97400" : "rgba(0,0,0,.18)", transition: "transform .2s ease, border-color .2s ease", "&:hover": { transform: "translateY(-3px)", borderColor: "#f9b233" } }}><CardActionArea type="button" aria-pressed={form.service_type === value} onClick={() => setField("service_type")({ target: { value } })} sx={{ height: "100%" }}><CardContent sx={{ textAlign: "center", py: 2 }}><Icon sx={{ fontSize: 34, mb: 0.5, color: "#fff" }} /><Typography fontWeight={800} sx={{ color: "#fff" }}>{label}</Typography><Typography variant="caption" aria-hidden="true" sx={{ display: "block", fontWeight: 700, visibility: form.service_type === value ? "visible" : "hidden" }}>Seleccionado</Typography></CardContent></CardActionArea></Card></Grid>)}</Grid>
    </Box>
