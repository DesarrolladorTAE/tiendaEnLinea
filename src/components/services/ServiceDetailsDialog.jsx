...
export default function ServiceDetailsDialog({ open, service, loading, onClose, onEdit }) {
  ...
  return (
    <Dialog open={open} onClose={onClose} fullScreen={fullScreen} fullWidth maxWidth="md" PaperProps={{ sx: { bgcolor: "#212529", color: "#fff", border: "1px solid #fff" } }}>
      ...
      </DialogContent><Divider sx={{ borderColor: "#495057" }} />
      <DialogActions sx={{ p: 2 }}><Button onClick={onClose} sx={{ color: "#fff" }}>Cerrar</Button><Button variant="outlined" onClick={() => onEdit(service)} disabled={loading} sx={{ color: "#fff", borderColor: "#fff" }}>Editar</Button></DialogActions>
    </Dialog>
  );
}
