...
const NEEDS_REFERENCE = ["td", "tc", "transferencia"];
const NEEDS_LAST4 = ["td", "tc", "transferencia"];
...
export default function PendingSalesPOS({ cambiarVista, posLocationId }) {
  ...
  const handleAddPayment = async () => {
    ...
    if (
      NEEDS_REFERENCE.includes(paymentForm.method) &&
      !paymentForm.referencia.trim()
    ) {
    ...
    if (
      NEEDS_LAST4.includes(paymentForm.method) &&
      paymentForm.ultimos_4.length !== 4
    ) {
      showError("Ingresa los 4 dígitos requeridos.");
      return;
    }
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
      ...
      <DialogContent dividers>
        <Stack spacing={2}>
          ...
          {sale.status === "open" && (
            ...
            >
              ...
              <Typography variant="body2" sx={{ color: "#6d4c41" }}>
                Esta venta aún no ha sido liquidada completamente. Se paga en el
          ...
          <Paper variant="outlined" sx={{ p: 2, borderRadius: 4 }}>
            <Typography sx={{ fontWeight: 950, mb: 1 }}>Productos</Typography>
