...
export default function FacturaWhatsAppDialog({
  ...
  const handleSend = async () => {
    if (!saleId || !tipoDocumento) {
      showError("No se encontró la venta o el tipo de documento.");
      return;
    }

    if (!usarTelefonoCliente && !phone.trim()) {
      const msg = "Debes capturar un número para envío externo.";
      setErrorMsg(msg);
      showError(msg);
      return;
    }
    ...
    try {
      ...
      const { data } = await axiosClientPOS.post(
      ...
      showSuccess(
        data?.message ||
          `Factura ${String(tipoDocumento).toUpperCase()} enviada correctamente por WhatsApp.`
      );
      ...
      // Si quieres ver todo más bonito desde tu helper:
      alertFromAxiosError(error, "No se pudo enviar la factura por WhatsApp");

      console.error("Error enviando WhatsApp:", {
        requestPayload: usarTelefonoCliente
          ? {
              tipo: tipoDocumento,
              es_cliente: true,
            }
  ...
  return (
    ...
    >
      ...
      <DialogContent dividers>
        <Stack spacing={2}>
          <Alert severity="info">
            Se enviará el archivo <b>{tipoDocumento?.toUpperCase()}</b> por
            WhatsApp.
          </Alert>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    {usarTelefonoCliente
                      ? telefonoCliente
                        ...
                        : "Se buscará el teléfono registrado del cliente."
          ...
          <TextField
            ...
            helperText={
              usarTelefonoCliente
                ? "Activado: se usará el teléfono del Cliente Registrado."
      ...
      <DialogActions sx={{ px: 3, py: 2 }}>
        ...
        <Button
          ...
          disabled={
            ...
            !saleId ||
            !tipoDocumento ||
            (!usarTelefonoCliente && !phone.trim())
          }
        >
